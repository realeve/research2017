<template>
  <div class="wrapper">
    <div class="tip">
      本次共收到问卷{{paper}}份，本数据统计延迟更新1分钟。
    </div>
    <div class="card" v-for="(question,i) of questionList" :key="i">
      <div class="content">
        <div class="weui-cells__title">{{i+1}}.{{question.title.split('（ ')[0].replace('请问您','你')}}</div>
  ...
  export default {
    ...
    watch: {
      researchData(data) {
        ...
        this.paper = parseInt(this.researchData[17][0].value)+parseInt(this.researchData[17][1].value);
      }
    },
    methods: {
      init() {
      ...
      getOption(data) {
        return {
          ...
          "color": ["#61A5E8", "#7ECF51", "#E16757", "#9570E5", "#605FF0", "#85ca36",
